...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score!=0:
        pos=opponent_score%10
        fun=digit_fn(pos)
        player_score=fun(player_score)
        opponent_score//=10
...
def hog_pile(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    pos1=player_score%10
    pos2=opponent_score%10
    if pos1==pos2:
        return pos1
    return 0
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0< goal and score1<goal:
        if next_player(who):
            ...
            score0+=take_turn(num_rolls0,score0,score1,dice,goal)
            ...
            score1+=take_turn(num_rolls1,score1,score0,dice,goal)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if diff>0:
        ...
        last_leader=0
        str_diff=str(diff)
        return 0,"Player 0 takes the lead by"+" "+str_diff
    if diff<0:
        if last_leader==1:
            return 1,None
        last_leader=1
        str_diff=str(abs(diff))
        return 1,"Player 1 takes the lead by"+" "+str_diff
    return None,None
    # END PROBLEM 6
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def average_call_original(*args):
        count=1
        sum=0
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    count=1
    final_nums_roll=0
    max_average_scores=0
    while count<=10:
        curr_average_scores=averaged_roll_dice(count,dice)
        if curr_average_scores>max_average_scores:
            max_average_scores=curr_average_scores
            final_nums_roll=count
        count+=1
    return final_nums_roll
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    new_score = score + hefty_hogs(score, opponent_score)
    if hog_pile(new_score, opponent_score) > 0:
        return 0
