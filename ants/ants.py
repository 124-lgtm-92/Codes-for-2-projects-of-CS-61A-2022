...
class Place:
    ...
    def __init__(self, name, exit_=None):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                assert place.ant is None, 'Two ants in {0}'.format(place)#若place.ant非None则断言错误
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i=0
        while not s.is_hive:
            if s.bees and self.min_range<=i<=self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in range(len(self.place.bees)):
            dic_bees[i]=self.place.bees[i]
        if self.health-amount<=0:
            for i in dic_bees.keys():
                dic_bees[i].reduce_health(amount+self.damage)   
        else:
            for i in dic_bees.keys():
                dic_bees[i].reduce_health(amount)   
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        victim_bee=random_bee(self.place.bees)
        if victim_bee:
            if self.chew_timer==0:
                self.chew_timer=self.time_to_chew
                victim_bee.reduce_health(victim_bee.health)
            else:
                self.chew_timer-=1
...
class TankAnt(ContainerAnt):
    ...
    def action(self,gamestate):
        ...
        for i in range(len(self.place.bees)):
            dic_bees[i]=self.place.bees[i]
        for i in dic_bees.keys():
            dic_bees[i].reduce_health(self.damage)  
class Water(Place):
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_exits:
            return None
        gamestate.queen_exits=True
    ...
    def action(self, gamestate):
        ...
        while curr:
            if curr.ant:
                if not curr.ant.is_double:
                    curr.ant.double()
                if curr.ant.is_container:
                    if curr.ant.ant_contained and not curr.ant.ant_contained.is_double:
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        def back_action():
            back_pos=self.place.entrance
            if self.blocked():
                self.sting(self.place.ant)
            elif self.health>0 and not back_pos.is_hive:
                self.move_to(back_pos)
            
        if self.slow_turns==0 and self.is_scared and self.scare_turns!=0:
            back_action()
            self.scare_turns-=1
        elif self.slow_turns!=0:
            if gamestate.time%2==0:
                if self.is_scared and self.scare_turns!=0:
                    back_action()
                    self.scare_turns-=1
                elif self.health > 0 and destination is not None:
                    self.move_to(destination)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while not curr.is_hive:
            if curr.ant:
                if curr.ant is not self:
                    front_insects[curr.ant]=curr_distance
                if curr.ant.is_container and curr.ant.ant_contained:
                    front_insects[curr.ant.ant_contained]=curr_distance
            if curr.bees:
                for bee in curr.bees:
                    front_insects[bee]=curr_distance
            curr_distance+=1
            curr=curr.entrance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        actual_damage=self.damage-distance*0.25-0.0625*self.insects_shot
        if actual_damage>0:
